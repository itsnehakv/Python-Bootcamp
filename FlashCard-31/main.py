...
'''Delete "words_to_learn.csv" for ALL french-eng words '''
try:
...
def next_card():
    ...
    window.after_cancel(flip_timer)  #so that it doesn't flip after EVERY 3 secs (we may be flipping through other cards as well)
...
def they_know():
    ...
    words_to_learn=pd.DataFrame(dict_fr_en)  #words excluding the removed words
    words_to_learn.to_csv("data/words_to_learn.csv",index=False)
    '''w/o this the dict_fr_en goes back to having all words. index=False makes
    sure that the index no. (from og "french_words.csv") doesn't keep on getting added to the words_to_learn.csv'''
    next_card()
...
flip_timer=window.after(3000,flip_card)   #global var
